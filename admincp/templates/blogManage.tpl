<div class="rightContHeading Heading">{$LANG.admin_blog_management} <a class="backButt pull-right" href="blogManage.php"><i class="fa fa-backward"></i> {$LANG.admin_back}</a></div>
<div class="ContBody clearfix">
<div class="row-fluid sky-form form-horizontal skyformbgNon">
		<div class="span6">
			{if $totalRecords gt 0}
				<!-- Sort By -->
				<form name="blogmanage" class="no-mar" method="post" action="blogManage.php" />
					Sort By <span class="restManageCol">:</span> <span class="selctTopAlign">
					<label class="select">
							<select class="no-mar" name="sortby" id="sortby" size="1" onchange="document.blogmanage.submit();">
		...
		<div class="span6">
			...
			<div class="processButton" id="searchkey">
				<form name="filterform" method="post" class="no-mar pull-left" onsubmit="return filterValidation();">
					...
					{$LANG.admin_keyword}:
					<input type="text" name="keyword" id="keyword" value="{$smarty.request.keyword}" class="no-mar" placeholder="blog name">
	...
	<div class="pageContTop  sky-form form-horizontal skyformbgNon">
		...
		<!--Button Right start-->
			<span class="pull-right">
				<a class="btn btn-success" href="blogManageAdd.php"><i class="fa fa-plus-circle"></i> {$LANG.admin_new}</a>
	...
	<div class="tableListContainer">
		<table width="100%" border="0" class="tableListContent">
			...
			{*	<td width="3%" align="center" class="listHeaderCont"><input type="checkbox" id="selectall" onclick="selectDeselectAll();" /></td>  *}
				...
				<td width="15%" align="" class="listHeaderCont">{$LANG.admin_blog_name}</td>					
				<td width="15%" align="center" class="listHeaderCont">{$LANG.admin_blog_description}</td>
			...
			{*	<td align="center" class="listCont"><input type="checkbox" class="case" value="{$followers_List[followers].blog_id}" onclick="individualSelect();" /></td>  *}
				<td align="center" class="listCont">{$followers_List[followers].sno}</td>
				<td align="left" class="listCont">{$followers_List[followers].blog_name|stripslashes}</td>				
				<td align="center" class="listCont">{$followers_List[followers].blog_description|stripslashes}</td>
                <td align="center" class="listCont">{$followers_List[followers].addeddate|date_format}</td>
				<td align="center" class="listCont" id="chgstatus{$followers_List[followers].blog_id}">
					{if $followers_List[followers].status eq '1'}
						<span onclick="return changeStatus('0','{$fieldname}','{$whereField}','{$tablename}','{$word}','{$followers_List[followers].blog_id}');" style="cursor:pointer;">
							<i class="fa fa-check-square-o"></i>
						</span>
						{else}
						<span onclick="return changeStatus('1','{$fieldname}','{$whereField}','{$tablename}','{$word}','{$followers_List[followers].blog_id}');" style="cursor:pointer;">
				...
				<td align="center" class="listCont">
					<span class="EditDeleteButton">
						<a href="blogManageAdd.php?blog_id={$followers_List[followers].blog_id}" >
							<i class="fa fa-edit"></i>
						</a>
					</span>
					<span class="EditDeleteButton">
						<span onclick="return changeStatus('delete','{$fieldname}','{$whereField}','{$tablename}','{$word}','{$followers_List[followers].blog_id}','banner');" style="cursor:pointer;">
