<div class="Heading">{$LANG.domain_name} <a class="backButt pull-right" href="domainManage.php"><i class="fa fa-backward"></i> {$LANG.admin_back}</a></div>
<div class="ContBody clearfix">
	 <div class="row-fluid sky-form form-horizontal skyformbgNon">
		<div class="span6">
			{if $totalRecords gt 0}
				<!-- Sort By -->
				<form name="domainmanage" class="no-mar" method="get" action="domainManage.php" />
					...
					<label class="select">
							<select class="no-mar" name="sortby" id="sortby" size="1" onchange="document.domainmanage.submit();">
								...
								<optgroup label="Status">
									<option value="active" {if $smarty.request.sortby eq active} selected="selected"{/if}>&nbsp;&nbsp;Publish</option>
									<option value="deactive" {if $smarty.request.sortby eq deactive} selected="selected"{/if}>&nbsp;&nbsp;Un Publish</option>
								</optgroup>
	...
	<div class="tableListContainer">
		<table width="100%" border="0" class="tableListContent">
				...
				<td width="10%" align="left" class="listHeaderCont">
					{if $smarty.get.sortby eq 'tdesc'}
						<a href="{$filename}?sortby=tasc{if $smarty.get.limit neq ''}&limit={$smarty.get.limit}{/if}{if $smarty.request.keyword neq ''}&keyword={$smarty.request.keyword}{/if}">
							{$LANG.admin_new_domain}<img src="images/icon_arrow_down.png" alt="Descending" title="Descending" />
						</a>
					{elseif $smarty.get.sortby eq 'tasc'}
						<a href="{$filename}?sortby=tdesc{if $smarty.get.limit neq ''}&limit={$smarty.get.limit}{/if}{if $smarty.request.keyword neq ''}&keyword={$smarty.request.keyword}{/if}">
							{$LANG.admin_new_domain}<img src="images/icon_arrow_up.png" alt="Ascending" title="Ascending" />
						</a>
					{else}
						<a href="{$filename}?sortby=tasc{if $smarty.get.limit neq ''}&limit={$smarty.get.limit}{/if}{if $smarty.request.keyword neq ''}&keyword={$smarty.request.keyword}{/if}">
							{$LANG.admin_new_domain}<img src="images/icon_arrow_up.png" alt="Ascending" title="Ascending" />
			...
			<tr {if $trvar is even}class="listLightGray"{/if} id="deletecate{$domaindetails[listing].newdomainid}">
				<td align="center" class="listCont"><input type="checkbox" class="case" value="{$domaindetails[listing].newdomainid}" onclick="return individualSelect();" />
				</td>
				<td align="center" class="listCont">{$domaindetails[listing].sno}</td>
				<td align="center" class="listCont">{$domaindetails[listing].domain_name}</td>
			...
			{*	<td align="center" class="listCont" id="chgstatus{$domaindetails[listing].id}">
					{if $domaindetails[listing].status eq 'publish'}
						<span onclick="return changeStatus('unpublish','{$fieldname}','{$whereField}','{$tablename}','{$word}','{$domaindetails[listing].newdomainid}');" style="cursor:pointer;">
							<i class="fa fa-check-square-o"></i>
						</span>
						{else}
						<span onclick="return changeStatus('publish','{$fieldname}','{$whereField}','{$tablename}','{$word}','{$domaindetails[listing].newdomainid}');" style="cursor:pointer;">
