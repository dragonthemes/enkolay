...
<div class="ContBody clearfix">
	...
	<div class="tableListContainer">
		<table width="100%" border="0" class="tableListContent">
				...
				<td width="10%" align="left" class="listHeaderCont">
					{if $smarty.get.sortby eq 'tdesc'}
						<a href="{$filename}?sortby=tasc{if $smarty.get.limit neq ''}&limit={$smarty.get.limit}{/if}{if $smarty.request.keyword neq ''}&keyword={$smarty.request.keyword}{/if}">
							Point Domain<img src="images/icon_arrow_down.png" alt="Descending" title="Descending" />
						</a>
					...
					{else}
						<a href="{$filename}?sortby=tasc{if $smarty.get.limit neq ''}&limit={$smarty.get.limit}{/if}{if $smarty.request.keyword neq ''}&keyword={$smarty.request.keyword}{/if}">
							Point Domain<img src="images/icon_arrow_up.png" alt="Ascending" title="Ascending" />
						</a>
					{/if}
				</td>
			...
			{section name="listing" loop="$pointdetails"}
			{assign var="trvar" value=$smarty.section.listing.rownum}
			<tr {if $trvar is even}class="listLightGray"{/if} id="deletecate{$pointdetails[listing].point_id}">
				<td align="center" class="listCont"><input type="checkbox" class="case" value="{$pointdetails[listing].point_id}" onclick="return individualSelect();" />
				</td>
				<td align="center" class="listCont">{$pointdetails[listing].sno}</td>
				<td align="center" class="listCont">{$pointdetails[listing].point_domain}</td>
				<td align="center" class="listCont">{$objCommon->getUserName($pointdetails[listing].user_id)}</td>
				<td align="center" class="listCont">{$objAdminMgmt->getDomain($pointdetails[listing].domain_id)}</td>
                <td align="center" class="listCont">{$pointdetails[listing].status}</td>
                <td align="center" class="listCont">{$pointdetails[listing].addeddate}</td>
				
				<!--Status-->
				<td align="center" class="listCont" id="chgstatus{$pointdetails[listing].point_id}">
					{if $pointdetails[listing].status eq 'pointed'}
						<span onclick="return changeStatus('unpointed','{$fieldname}','{$whereField}','{$tablename}','{$word}','{$pointdetails[listing].point_id}');" style="cursor:pointer;">
							<i class="fa fa-check-square-o"></i>
						</span>
						{else}
						<span onclick="return changeStatus('pointed','{$fieldname}','{$whereField}','{$tablename}','{$word}','{$pointdetails[listing].point_id}');" style="cursor:pointer;">
	...
	<div class="pageContBott sky-form form-horizontal skyformbgNon">
		...
		<!--Button List start-->
			<div class="pull-right">
				 <input type="button"  class="manageButton but_activate" value="Pointed" onclick="sendMailToUsersPublish('pointed','{$fieldname}','{$whereField}','{$tablename}','{$word}');" style="display:none;"/>
			<input type="button" class="manageButton but_deactivate" value="UnPointed"  onclick="sendMailToUsersPublish('unpointed','{$fieldname}','{$whereField}','{$tablename}','{$word}');" style="display:none;"/>
				<input type="button" class="manageButton but_delete" value="Delete" style="display:none;" onclick="adminActivateDeactivate('delete','deletefield','{$whereField}','{$tablename}','{$word}','Point Domain');" />
				<input type="button" class="manageButton send_mail" value="Send Mail" style="display:none;" onclick="sendMailToUsers('1','mail_status','{$whereField}','{$tablename}','{$word}','Point Domain');" />
			</div>
		<!--Button List End-->
	</div>
</div>