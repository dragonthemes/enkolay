<div class="rightContHeading Heading">{$LANG.admin_store_management} <a class="backButt pull-right" href="storeManage.php"><i class="fa fa-backward"></i> {$LANG.admin_back}</a></div>
<div class="ContBody clearfix">
	<div class="row-fluid sky-form form-horizontal skyformbgNon">
		<div class="span6">
			{if $totalRecords gt 0}
				<!-- Sort By -->
				<form name="storemanage" class="no-mar" method="post" action="storeManage.php" />
					Sort By <span class="restManageCol">:</span> <span class="selctTopAlign">
					<label class="select">
							<select class="no-mar" name="sortby" id="sortby" size="1" onchange="document.storemanage.submit();">
								...
								<optgroup label="Others">
									<option value="casc" {if $smarty.request.sortby eq 'casc'}selected="selected"{/if}>&nbsp;&nbsp;Store Name A to Z</option>
									<option value="cdesc" {if $smarty.request.sortby eq 'cdesc'}selected="selected"{/if}>&nbsp;&nbsp;Store Name Z to A</option>
		...
		<div class="span6">
			...
			<div class="processButton" id="searchkey">
				<form name="filterform" method="post" class="no-mar pull-left" onsubmit="return filterValidation();">
					...
					{$LANG.admin_keyword}:
					<input type="text" name="keyword" id="keyword" value="{$smarty.request.keyword}" class="no-mar" placeholder="Store name">
	...
	<div class="pageContTop">
		...
		<!--Button Right start-->
			<span class="pull-right marTop10">
				<a class="btn btn-success" href="storeManageAdd.php"><i class="fa fa-plus-circle"></i> {$LANG.admin_new}</a>
				...
				<input type="button" class="manageButton but_delete" value="Delete" style="display:none;" onclick="adminActivateDeactivate('delete','deletefield','{$whereField}','{$tablename}','{$word}','Store');" />
			</span>
		<!--Button Right End--> 
	</div>
		<!--Error Msg-->
	...
	<div class="tableListContainer">
		<table width="100%" border="0" class="tableListContent">
			<tr class="listHeader leftNavblueBg">
				...
				<td width="15%" align="" class="listHeaderCont">{$LANG.admin_store_name}</td>					
				<td width="15%" align="center" class="listHeaderCont">{$LANG.admin_store_description}</td>				
				<td width="5%" align="center" class="listHeaderCont">{$LANG.admin_store_status}</td>
				<td width="10%" align="center" class="listHeaderCont">{$LANG.admin_action}</td>					
			</tr>
			{section name="store" loop="$store_List"}			
			{assign var="trvar" value=$smarty.section.store.rownum}
			<tr {if $trvar is even}class="listLightGray"{/if}>
				<td align="center" class="listCont"><input type="checkbox" class="case" value="{$store_List[store].store_id}" onclick="individualSelect();" /></td>
				<td align="center" class="listCont">{$store_List[store].sno}</td>
				<td align="left" class="listCont">{$store_List[store].store_name|stripslashes}</td>				
				<td align="center" class="listCont">{$store_List[store].store_description|stripslashes}</td>
				<td align="center" class="listCont" id="chgstatus{$store_List[store].store_id}">
					{if $store_List[store].status eq '1'}
					
					<span onclick="return changeStatus('0','{$fieldname}','{$whereField}','{$tablename}','{$word}','{$store_List[store].store_id}');" style="cursor:pointer;">
						<i class="fa fa-check-square-o"></i>
					</span>
					{else}
					<span onclick="return changeStatus('1','{$fieldname}','{$whereField}','{$tablename}','{$word}','{$store_List[store].store_id}');" style="cursor:pointer;">
				...
				<td align="center" class="listCont">
					<span class="EditDeleteButton">
						<a href="storeManageAdd.php?store_id={$store_List[store].store_id}" >
							<i class="fa fa-edit"></i>
						</a>
					</span>
					<span class="EditDeleteButton">
						<span onclick="return changeStatus('delete','{$fieldname}','{$whereField}','{$tablename}','{$word}','{$store_List[store].store_id}','banner');" style="cursor:pointer;">
