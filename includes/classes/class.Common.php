		    ...
		    if(isset($_SESSION['domain_id']) && !empty($_SESSION['domain_id']))
                {
        			...
        			if($UpResult)
        				{
        					$objSmarty->assign('successmsg','E-postanız Başarıyla Güncellendi');
        				}
                }			
		}
/**
						...
						if($res_ins)
							{
								...
								$mail_content		=	$url;
								$from_name 			=   $CFG['site']['site_main_domain'];
								$mail_content      .=   "<br>Kullanıcı Adı: ".$email;
								$mail_content      .=   "<br>Şifre: ".$password; 
								$mailsubject  = $CFG['site']['sitename']." ".$_lang['addRegister_sub'];
								$mail_content = $this->readfilecontent($CFG['site']['emailtpl_path']."/emailRegistration.tpl");
						        $mail_content = str_replace('{SITE_URL}',$CFG['site']['base_url'],$mail_content);
						        $mail_content = str_replace('{STATUS}','Merhaba '.$username. ',',$mail_content);
						        $mail_content = str_replace('{INFO}','Enkolayweb.com\'a kayıt olduğunuz için teşekkür ederiz,',$mail_content);
						        $mail_content = str_replace('{SITE_TITLE}',$CFG['site']['site_main_domain'],$mail_content);
						        $mail_content = str_replace('{SITE_LOGO}',$CFG['site']['logoname'],$mail_content);
						        ...
						        $mail_content = str_replace('{DETAILS}','Bilgileriniz aşağıdadır',$mail_content);
						        $mail_content = str_replace('{ADMIN}','Yönetici',$mail_content);  
							 	...
							 	if($ok)
									{
										...
										$mail_body = str_replace('{STATUS}','Merhaba Yönetici,',$mail_body);
										$mail_body = str_replace('{DETAILS}','Kullanıcı detayları aşağıdadır',$mail_body);
										$mail_body = str_replace('{INFO}','Kullanıcı sitenize başarıyla kayıt oldu,',$mail_body);
								        ...
								        $mail_body = str_replace('{ADMIN}',$CFG['site']['site_main_domain'].' Ekibi',$mail_body);
...
function chkUserLogin()
    {
		...
		if($user_email)
			{
				$num_user 	= $this->getNumvalues($CFG['table']['register'],"`email`='".$user_email."' AND `password` ='".$Password."' AND `log_status` = '2' 
                              AND `delete_status` != 'deleted' ");
		 
				$valid_user	= $this->getNumvalues($CFG['table']['register'],"`email` ='".$user_email."' AND `password` ='".$Password."' 
                              AND `log_status` = '1' AND `delete_status` != 'deleted'");
				
				$userId     = $this->getValue("user_id",$CFG['table']['register'],"`email` ='".$user_email."' AND `password` ='".$Password."' LIMIT 1");
				 
				$delStatus  = $this->getValue("delete_status",$CFG['table']['register'],"`email` ='".$user_email."'  AND `password` ='".$Password."' LIMIT 1");
  	...
  	function forgetPass(){
		...
		if(count($forgetpass[0]['password']) > 0 && count($forgetpass[0]['password']) != 0){
			if($forgetpass[0]['log_status'] == '2'){
				...
				$mail_content		=	$url." Şifreniz : ".$password;
				$mailsubject  		= 	$CFG['site']['sitename'].": ".$_lang['forgetPass_sub'];
		        ...
		        $mail_content = str_replace('{YOURNAME}','Yönetici',$mail_content);
		        $mail_content = str_replace('{PASSWORD}',$password,$mail_content);
		        $mail_content = str_replace('{USERNAME}',$username,$mail_content);
	...
	function updatePassword()
		{
			...
			$res_regupdate	=	$this->ExecuteQuery($sql_regupdate,'update');
			if($res_regupdate){
			$objSmarty->assign('successmsg','Şifreniz Başarıyla Güncellendi');
			}			
		}
            ...
            if(isset($_SESSION['user_id']) && !empty($_SESSION['user_id']))
                {
                    $email_already =$this->getValue("email",$CFG['table']['register'],"email='".$this->filterInput($_POST['email'])."' AND user_id != '".$this->filterInput($_SESSION['user_id'])."'");
        		 	if($email_already)
        				{
        					echo "Bu e-posta adresi zaten sistene kayıtlı, lütfen başka bir adres deneyin veya bu adresle giriş yapmayı deneyin";
        					exit();	
        				}
        			else
        				{
...
function InsertSuggestion()
	{
        ...
        if(isset($_SESSION['user_id']) && !empty($_SESSION['user_id']))
            {
                $UpQuery  = "INSERT ".$CFG['table']['suggesstion']." SET `suggesstion_descrip` = '".$this->filterInput($_POST['deleteopnion'])."',
                             `user_id` ='".$this->filterInput($_SESSION['user_id'])."' , `addeddate` = NOW()"; 
    			$UpResult = mysql_query($UpQuery) or die($this->mysql_error($UpQuery));
    			return true;
            }	
	}	
			...
			if($_SESSION['theme'] && isset($_SESSION['user_id']) && !empty($_SESSION['user_id']))
				{
			        ...
			        $UpQuery = "INSERT ".$CFG['table']['domaindetails']." SET subdomainurl = '".$this->filterInput($_POST['subdomain_url'])."',site_title = 'Benim Sitem', domain_type = '".$_POST['type']."', user_id ='".$this->filterInput($_SESSION['user_id'])."' ,theme_id ='".$this->filterInput($_POST['theme_id'])."',validfromdate = '".time()."', validtodate = '".$validtotimestr."', addeddate = NOW()$conditon"; 
                    $UpResult =	$this->ExecuteQuery($UpQuery,'insert');
                    $LastInsertedRow = mysql_insert_id();
                    
                    $temp_query = "INSERT ".$CFG['table']['temp_domaindetails']." SET domain_id = '".$this->filterInput($UpResult)."',subdomainurl = '".$this->filterInput($_POST['subdomain_url'])."',site_title = 'Benim Sitem', domain_type = '".$_POST['type']."', user_id ='".$this->filterInput($_SESSION['user_id'])."' ,theme_id ='".$this->filterInput($_POST['theme_id'])."',validfromdate = '".time()."', validtodate = '".$validtotimestr."',addeddate = NOW()$conditon";
					...
					$page_id   = $this->InsertPageDetails($pagename='Ana Sayfa',$seo_title='ana-sayfa',$page_layout='short',$page_desc='ana sayfa',$meta_key='Ana Sayfa',$blog_comment='0',$store_comment='0',$setmain='Y');
					//$this->defaultpageList($UpResult,$page_id,$_POST['theme_id']);
                    $this->InsertPageDetails($pagename='Hakkımızda',$seo_title='hakkimizda',$page_layout='tall',$page_desc='hakkimizda',$meta_key='hakkımızda',$blog_comment='0',$store_comment='0',$setmain='N');
					$this->InsertPageDetails($pagename='Bize Ulaşın',$seo_title='bize-ulasin',$page_layout='tall',$page_desc='Bize Ulaşın',$meta_key='bize ulaşın',$blog_comment='0',$store_comment='0',$setmain='N');
			...
			elseif($_SESSION['blog'] && isset($_SESSION['user_id']) && !empty($_SESSION['user_id']))
				{
				   ...
				   $UpQuery = "INSERT ".$CFG['table']['domaindetails']." SET subdomainurl = '".$this->filterInput($_POST['subdomain_url'])."',site_title = 'Benim Sitem', domain_type = 'SD',user_id ='".$this->filterInput($_SESSION['user_id'])."' ,blog_id ='".$this->filterInput($_POST['blog_id'])."', validfromdate = '".time()."', validtodate = '".$validtotimestr."', addeddate = NOW()$conditon"; 
				   $UpResult =	$this->ExecuteQuery($UpQuery,'insert');
					...
					$this->InsertPageDetails($pagename='Hakkımızda',$seo_title='hakkimizda',$page_layout='tall',$page_desc='Hakkımızda',$meta_key='hakkımızda',$blog_comment='0',$store_comment='0',$setmain='N');
					$this->InsertPageDetails($pagename='Bize Ulaşın',$seo_title='bize-ulasin',$page_layout='tall',$page_desc='Bize Ulaşın',$meta_key='bize ulaşın',$blog_comment='0',$store_comment='0',$setmain='N');
	...
	function sendMailForTheme($password)
		{
            ...
            if(isset($password) && !empty($password))
                 $PWD_CONDENT = '<tr>
				<td style="color: #333333; font-family: Trebuchet MS; font-size: 14px; padding: 10px 0 0 40px; text-align: left;"><b style="width:100px;display: inline-block;">Site Önizleme Şifresi</b> : "'.$password.'"</td>
			</tr>';
            
			
			$mailsubject=$_lang['sendMailForTheme_sub'];
	        ...
	        $mail_content = str_replace('{STATUS}','Merhaba! Yeni web sitenizi '.$CFG['site']['site_main_domain'].' ile hemen düzenlemeye başlayabilirsiniz.',$mail_content);
	        $mail_content = str_replace('{SITE_LOGO}',$CFG['site']['logoname'],$mail_content);
	        $mail_content = str_replace('{INVITE_URL}',$CFG['site']['base_url'].'/images/inviitemail.png',$mail_content);
	        $mail_content = str_replace('{SINCERELY}','Saygılarımızla,',$mail_content);
	        $mail_content = str_replace('{TEAM}',$CFG['site']['site_main_domain'].' Ekibi',$mail_content);
	        $mail_content = str_replace('{INFO}','Merhaba '.$username.',',$mail_content);
			$mail_content = str_replace('{INFO_LEFT}','Enkolayweb.com ile yeni bir web sitesi oluşturduğunuz için tebrik ederiz. Sitenize bu adresten ulaşabilirsiniz: ',$mail_content);
            $mail_content = str_replace('{PWD_CONT}',$PWD_CONDENT,$mail_content);
			$mail_content = str_replace('{INFO_LINK}',$subdomainurl,$mail_content);
			$mail_content = str_replace('{INFO_RIGHT}','Sitenizi güzel günlerde kullanmanızı ve güncellemenizi dileriz...',$mail_content);
	        //echo $mail_content;
	        ...
	        if($ok)
	        	{
					...
					$mail_body    = str_replace('{SITE_LOGO}',$CFG['site']['logoname'],$mail_body);
					$mail_body    = str_replace('{INVITE_URL}',$CFG['site']['base_url'].'/images/inviitemail.png',$mail_body);
					$mail_body    = str_replace('{STATUS}','Merhaba, Sizi bilgilendirmekten mutluluk duyarız ki;',$mail_body);
			        $mail_body    = str_replace('{INFO}','Merhaba Yönetici,',$mail_body);
			        $mail_body    = str_replace('{INFO_LEFT}',$username.' sisteminiz üzerinden yeni bir site oluşturdu',$mail_body);
			        ...
			        $mail_body    = str_replace('{SINCERELY}','Saygılarımızla,',$mail_body);
			        $mail_body    = str_replace('{TEAM}',$CFG['site']['site_main_domain'].' Ekibi',$mail_body);
                    $mail_body    = str_replace('{SITE_MAIN_DOMAIN}',$CFG['site']['site_main_domain'],$mail_body);
	...
	function sendMailForBlog($password)
		{
            ...
            if(isset($password) && !empty($password))
                 $PWD_CONDENT = '<tr>
				<td style="color: #333333; font-family: Trebuchet MS; font-size: 14px; padding: 10px 0 0 40px; text-align: left;"><b style="width:100px;display: inline-block;">Site Önizleme Şifreniz</b> : "'.$password.'"</td>
			</tr>';

            
			$mailsubject=$_lang['sendMailForBlog_sub'] ;
			$name=$CFG['site']['sitename'];
			$username=$this->getUserName($this->filterInput($_SESSION['user_id']));
			$to_email= $this->getValue("email",$CFG['table']['register'],"username='".$this->filterInput($username)."'");
			$from_email= $this->getValue("admin_email",$CFG['table']['sitesetting'],"domain_id = '1'");
	        ...
	        $mail_content = str_replace('{STATUS}','Merhaba! Yeni web blogunuzu '.$CFG['site']['site_main_domain'].'ile hemen düzenlemeye başlayabilirsiniz.',$mail_content);
	        $mail_content = str_replace('{SITE_LOGO}',$CFG['site']['logoname'],$mail_content);
	        $mail_content = str_replace('{INVITE_URL}',$CFG['site']['base_url'].'/images/inviitemail.png',$mail_content);
	        $mail_content = str_replace('{SINCERELY}','Saygılarımızla,',$mail_content);
	        $mail_content = str_replace('{TEAM}',$CFG['site']['site_main_domain'].' Ekibi',$mail_content);
	        $mail_content = str_replace('{INFO}','Merhaba '.$username.',',$mail_content);
            $mail_content = str_replace('{INFO_LEFT}','Enkolayweb.com ile yeni bir web blog oluşturduğunuz için tebrik ederiz. Blogunuza bu adresten ulaşabilirsiniz:',$mail_content);
			$mail_content = str_replace('{INFO_LINK}',$subdomainurl,$mail_content);
			$mail_content = str_replace('{INFO_RIGHT}','Blogunuzu güzel günlerde kullanmanızı ve güncellemenizi dileriz...',$mail_content);
			//echo $mail_content; die();
	       $ok=$this->sendMail($name,$from_email,$to_email,$mailsubject,$mail_content);
	       $ok=1;
	        if($ok)
	        	{
					...
					$mail_body    = str_replace('{STATUS}','Merhaba, Sizi bilgilendirmekten mutluluk duyarız ki;',$mail_body);
			        $mail_body    = str_replace('{INFO}','Merhaba Yönetici,',$mail_body);
			        $mail_body    = str_replace('{INFO_LEFT}',$username.' sisteminiz üzerinden yeni bir blog oluşturdu',$mail_body);
					$mail_body    = str_replace('{INFO_LINK}','',$mail_body);
					$mail_body    = str_replace('{INFO_RIGHT}','',$mail_body);
			        $mail_body    = str_replace('{INFO_CONTENT}','',$mail_body);
	...
	function InsertPageDetails($pagename,$seo_title,$page_layout,$page_desc,$meta_key,$blog_comments,$store_comment,$setmain)
		{
			...
			if(isset($pagename) && !empty($pagename) && $pagename == 'Ana Sayfa')
			    $banner_status = "banner_status = 'need'";
			 else
			 	$banner_status = "banner_status = 'no_need'";

          	 $UpQuery = "INSERT ".$CFG['table']['temp_pages']." SET domain_id = '".$this->filterInput($_SESSION['domain_id'])."',pagename ='".$this->filterInput($pagename)."' ,seo_title ='".$this->filterInput($seo_title)."',page_layout ='".$this->filterInput($page_layout)."',page_desc ='".$this->filterInput($page_desc)."',meta_key ='".$this->filterInput($meta_key)."',blog_comments= '".$this->filterInput($blog_comments)."',store_comment= '".$this->filterInput($store_comment)."',main_page='".$setmain."',publish_status='U',".$banner_status." "; 
	...
	function sendMailToCommentedUser()
		{
			...
			$mailsubject  = $CFG['site']['sitename']." ".$_lang['sendMailToCommentedUser_sub'] ;
			...
			$mail_body = str_replace('{STATUS}','blog yazınıza bir yorum yaptı,',$mail_body);
	...
	function sendMailToContactForm($domain_id)
		{
			...
			//$sender_email	=	$_POST['email'];
			$sender_email	=	"postaci@enkolayweb.com";		
			$to_name = $_POST['firstname'].' '.$_POST['lastname'];		
			$commentdetails = $_POST['comment_contact'];			
			$mailsubject  = $CFG['site']['sitename']." ".$_lang['sendMailToContactForm_sub'];
			...
			if($header_logo_config == '2')
				{
					...
					$mail_body = str_replace('{SITE_LOGO}','<img width="210" height="auto" src='.$logo.'  alt='.$logo.'>',$mail_body);
				}
	...
	function addPage()
		{
            ...
            if(isset($_SESSION['domain_id']) && !empty($_SESSION['domain_id']))
                {
                    $UpQuery  = "INSERT INTO ".$CFG['table']['temp_pages']." SET domain_id= '".$this->filterInput($_SESSION['domain_id'])."', pagename = 'untitled',seo_title = 'untitled' , publish_status='U',banner_status='no_need'";
        			$res_qry = $this->ExecuteQuery($UpQuery,'insert');
        			return $res_qry;
                }			
		}
	...
	*/
	...
	function insertMailDetailsForContact($userid,$pagelistid)
            ...
            if($userid != '' && $pagelistid != '' && $_GET['pageid'] != '' && $_GET['domainid'] != '')
                {
        		    ...
        		    $UpQuery  = "INSERT ".$CFG['table']['temp_contact_form']." SET page_id = '".$this->filterInput($_GET['pageid'])."',page_list_id = '".$this->filterInput($pagelistid)."',domain_id = '".$this->filterInput($_GET['domainid'])."',mail_to ='".$this->filterInput($email)."',text1 = 'Ad' ,text2 = 'Soyad' ,text3 = 'E-posta',text4 = 'Mesaj',text5='Gönder',added_date = NOW(),publish_status='U'"; 
        			$UpResult	=	$this->ExecuteQuery($UpQuery,'insert');
        			return true;
                }		 	
		}
		...
		function getImage($page_id)
			{
				global $CFG,$objSmarty;				
				$image = $this->getValue('image_name',$CFG['table']['temp_single_images'],"page_list_id = '".$this->filterInput($page_id)."'");				
                $objSmarty->assign('images',$image);
		...
		function getImageText($page_id)
			{
				global $CFG,$objSmarty;
			 	$image = $this->getValue('image_name',$CFG['table']['temp_imagetxt_images'],"page_list_id = '".$this->filterInput($page_id)."'");
				$objSmarty->assign('images',$image);
			}
       ...
       function getGoogleImage($domain_id,$page_id,$page_list_id)
		...
		function getImageForShowTop($domain_id,$status='domainlogo')
			{
				global $CFG,$objSmarty;
				$image = $this->getValue('temp_image_name',$CFG['table']['domain_images'],"domain_id = '".$this->filterInput($domain_id)."' and status ='".$this->filterInput($status)."'");
                ...
                if($_GET['pageid'] != '' && $_GET['domainid'] != '')
                    {
                        ...
                        $maxsortid++;                        
        		        $UpQuery  = "INSERT ".$CFG['table']['temp_pagelist']." SET  
        		        						page_id           = '".$this->filterInput($_GET['pageid'])."',
        		        						domain_id         = '".$this->filterInput($_GET['domainid'])."',
        		        						publish_status    = 'U',
        		        						".$_GET['name']." = '1',
        		        						column_id         = '".$this->filterInput($_GET['column_id'])."',
        		        						column_pagelist   = '".$this->filterInput($_GET['columnpagelist'])."'"; 
                        $UpResult1	=	$this->ExecuteQuery($UpQuery,'insert');
   ...
   */
	...
	function updatePublishPage()
		{
           ...
           if(isset($domain_det_res) && $domain_det_res[0]['publish'] == 'Y')   
                {                   
                    ...
                    #Column Text image 
                    #---------------------------------------------------------------------------------------------------------------
                    $this->columnImageTextProcess($_POST['domain_id']);
                    #---------------------------------------------------------------------------------------------------------------
               }           
		 }
	...
	*/
    ...
    function getGalleryImagesValue($domain_id,$tablename)
        {
            global $CFG;
            $qry_det = "SELECT gallery_id, domain_id,imagelib_id, page_id, page_list_id,  gallery_name,gallery_name_thumb, publish_status FROM ".$tablename." WHERE domain_id ='".$this->filterInput($domain_id)."'";
            $res = $this->ExecuteQuery($qry_det,'select');
            return $res;
        }
    #..........................................................................................................................   
	...
	*/
    ...
    function getSliderImagesValue($domain_id,$tablename)
        {
            global $CFG;
            $qry_det = "SELECT slider_id, domain_id,imagelib_id, page_id, page_list_id,slider_images,publish_status FROM ".$tablename." WHERE domain_id = '".$this->filterInput($domain_id)."' ";
            $res = $this->ExecuteQuery($qry_det,'select');
            return $res;
        }
    #................................................................................................................................
                    ...
                    if($temp_slider_val[$sub]['publish_status'] == 'U')
                        {
                             $upd_qry =  " INSERT INTO ".$CFG['table']['slider_images']." SET ".
                                ...
                                " imagelib_id          = '".$this->filterInput($temp_slider_val[$sub]['imagelib_id'])."',".
	...
	*/
    #delete banner slider images table if temp_banner_slider_iamges table is empty
     ...
     function getBanSliderImagesValue($domain_id,$tablename)
        {
            global $CFG;
            $qry_det = "SELECT banner_slider_id, domain_id,imagelib_id, page_id, image_name,status, publish_status FROM ".$tablename." WHERE domain_id ='".$this->filterInput($domain_id)."'";
            $res = $this->ExecuteQuery($qry_det,'select');
            return $res;
        }
    #................................................................................................................................
                     ...
                     if($temp_banslider_val[$sub]['publish_status'] == 'U')
                        {
                             $upd_qry =  " INSERT INTO ".$CFG['table']['banner_slider_images']." SET ".
                                ...
                                " imagelib_id            = '".$this->filterInput($temp_banslider_val[$sub]['imagelib_id'])."',".
	...
	*/
    ...
    function getSingleImagesValue($domain_id,$tablename)
        {
            global $CFG;
            $qry_det = "SELECT img_id, domain_id,imagelib_id,  page_id,page_list_id, image_name,status,alignment, publish_status FROM ".$tablename." WHERE domain_id ='".$this->filterInput($domain_id)."'";
            $res = $this->ExecuteQuery($qry_det,'select');
            return $res;
        }
    #................................................................................................................................
   ...
   #for get temmp_imagetxt_images table value
    ...
    function getImageTxtValue($domain_id,$tablename)
        {
            global $CFG;
            $qry_det = "SELECT img_id, domain_id,imagelib_id,  page_id, page_list_id, image_name, status, publish_status FROM ".$tablename." WHERE domain_id ='".$this->filterInput($domain_id)."'";
            $res = $this->ExecuteQuery($qry_det,'select');
            return $res;
        }
    #................................................................................................................................
                    ...
                    if($temp_imgtxt_val[$sub]['publish_status'] == 'U')
                        {
                            $ins_qry =  " INSERT INTO ".$CFG['table']['imagetxt_images']." SET ".
                                        ...
                                        " imagelib_id            = '".$this->filterInput($temp_imgtxt_val[$sub]['imagelib_id'])."',".
	...
	*/
    ...
    function getDomainImagesValue($domain_id,$tablename)
        {
            global $CFG;
            $qry_det = "SELECT img_id,temp_image_name,image_name,imagelib_id   FROM ".$tablename." ".
                                  " WHERE domain_id ='".$this->filterInput($domain_id)."'";
            $res = $this->ExecuteQuery($qry_det,'select');
            return $res;
        }  
...
function pageListPublishProcess($domain_id)
    {
        ...
        $temp_upd_id = '';
        //echo "<pre>";print_r($temp_pagelist_val);echo "</pre>";
                       ...
                       if( !in_array($temp_pagelist_val[$sub]['pagelist'],$cur_main_pagelist_id) )
                            {  
                               $update_qry = "INSERT INTO ".$CFG['table']['page_list']." SET  
                                             ...
                                             team_with_text 	                    ='".$temp_pagelist_val[$sub]['team_with_text']."', 
                                             column_image_show 	                    ='".$temp_pagelist_val[$sub]['column_image_show']."' "; 
            ...
            if(isset($_SESSION['user_id']) && $_POST['domain_id'] != '')
                {
        			...
        			$domain_name = $this->getValue("subdomainurl",$CFG['table']['domaindetails'],"domain_id='".$this->filterInput($_POST['domain_id'])."'");
        					...
        					$mailsubject  = $CFG['site']['sitename']." Alan Adı Eşleştirme Detayları ";
        					$mail_content = $this->readfilecontent($CFG['site']['emailtpl_path']."/emailPointDomain.tpl");
        					$mail_content = str_replace('{SITE_TITLE}',$CFG['site']['site_main_domain'],$mail_content);
        					$mail_content = str_replace('{SITE_LOGO}',$CFG['site']['logoname'],$mail_content);
        			        $mail_content = str_replace('{STATUS}','"'.$CFG['site']['base_url'].'" sitesinin "'.$point_domain.'" alan adıyla eşlenmesi istendi. Lütfen DNS ayarlarınızı yapınız',$mail_content);
                            $mail_content = str_replace('{SITE_MAIN_DOMAIN}',$CFG['site']['site_main_domain'],$mail_content);
        					$ok=$this->sendMail($CFG['site']['site_main_domain'],$from_email,$to_email,$mailsubject,$mail_content);
        			$objSmarty->assign('ErrorMessage','Alan Adı Başarıyla Eklendi');
                }			
		}
	/**
			 	...
			 	if($reset_email[0]['log_status'] == '2')
					{
						...
						$from_email			= 	$this->getValue("admin_email",$CFG['table']['sitesetting'],"domain_id='1'");
						$mailsubject  = $CFG['site']['sitename']." Şifrenizi Sıfırlayın ";
						$mail_content = $this->readfilecontent($CFG['site']['emailtpl_path']."/emailResetPass.tpl");
						$mail_content = str_replace('{SITE_TITLE}',$CFG['site']['site_main_domain'],$mail_content);
						$mail_content = str_replace('{SITE_LOGO}',$CFG['site']['logoname'],$mail_content);
				        $mail_content = str_replace('{INIT}','Merhaba,',$mail_content);
				        $mail_content = str_replace('{STATUS_ONE}','Hesabınız için bir şifre sıfırlama talebinde bulunuldu.',$mail_content);
				        $mail_content = str_replace('{STATUS_TWO}','Şifrenizi sıfırlamak istiyorsanız lütfen aşağıdaki bağlantıyı takip edin. Eğer böyle bir talepte bulunmadıysanız veya ihtiyacınız kalmadıysa bu mesajı gözardı edebilirsiniz.',$mail_content);
				        $mail_content = str_replace('{SINCERELY}','Saygılarımızla,',$mail_content);
				        $mail_content = str_replace('{TEAM}',$CFG['site']['site_main_domain'].' Ekibi',$mail_content);
	...
	function EmailForForgotUser()
		...
		if(count($forgetpass[0]['password']) > 0 && count($forgetpass[0]['password']) != 0){
			...
			$from_email			= 	$this->getValue("admin_email",$CFG['table']['sitesetting'],"domain_id='1'");
			$mailsubject  = $CFG['site']['sitename']." ".$_lang['email_forget_sub'] ;
			$mail_content = $this->readfilecontent($CFG['site']['emailtpl_path']."/emailResetPass.tpl");
			$mail_content = str_replace('{SITE_TITLE}',$CFG['site']['site_main_domain'],$mail_content);
			$mail_content = str_replace('{SITE_LOGO}',$CFG['site']['logoname'],$mail_content);
	        $mail_content = str_replace('{INIT}','Merhaba,',$mail_content);
	...
	public function getGalleryImage($domain_id,$page_id,$page_list_id)
		{
			global $CFG,$objSmarty;
		    $sqlsel = "SELECT gallery_name,gallery_name_thumb,gallery_id FROM ".$CFG['table']['temp_gallery_images']. " WHERE page_list_id = '".$this->filterInput($page_list_id)."' AND domain_id ='".$this->filterInput($domain_id)."' AND page_id ='".$this->filterInput($page_id)."' order by gallery_id asc";
			$sqlres =  $this->ExecuteQuery($sqlsel,'select');
	...
	public function getSliderImageNew($page_list_id)
		{
			global $CFG,$objSmarty;
		    $sqlsel = "SELECT slider_images,slider_id,page_list_id,page_id FROM ".$CFG['table']['temp_slider_images']. " WHERE page_list_id = '".$this->filterInput($page_list_id)."' AND domain_id ='".$this->filterInput($_SESSION['domain_id'])."' AND page_id ='".$this->filterInput($_SESSION['page_id'])."'";
			$sqlres =  $this->ExecuteQuery($sqlsel,'select');
            ...
            if($_POST['domain_id'] != '' && $_POST['img_id'] != '')
                {
                    ...
                    //$UpQuery  = "UPDATE  ".$CFG['table']['temp_banner_slider_images']." SET  WHERE banner_slider_id ='".$this->filterInput($_POST['img_id'])."'";
        			$UpResult = mysql_query($UpQuery) or die($this->mysql_error($UpQuery));
                    $numval = $this->getNumvalues($CFG['table']['temp_banner_slider_images']," domain_id='".$_SESSION['domain_id']."' ");
                    if($numval==0)
		...
		public function updateSubscriptionDate($domain_id,$txn_id)
           {
               ...
               $mail_content = '<html>
                                		...
                                		<table width="760" border="0" align="center" cellpadding="5" cellspacing="0" style="border:1px solid #CCCCCC; background:#ffffff; box-shadow:0 0 5px #B9B9B9;">
                                			<tr>
                                				<td align="center" colspan="3" style="background: #87CEEB; display: block; text-align: center;">
                                					<img width="210" height="auto" src='.$CFG['site']['logoname'].' alt='.$CFG['site']['sitename'].'/>
                                				</td>
                                			</tr>
                                			<tr>
                                				<td style="color: #0F0; font-family: Trebuchet MS; font-size: 14px; font-weight: bold; padding: 10px 0 0; text-align: center;">
                                					<span style="display: block; margin: 0px 10px; padding: 10px 0px;">
                                						Ödemenizin Detayları.
                                					</span>
                                				</td>				
                                			</tr>
                                            
                                			<tr>
                                			 	<td style="color: #333333; font-family: Trebuchet MS; font-size: 14px; font-weight: bold; padding: 10px 0 0 10px; text-align: left;">Merhaba '.$res[0]['username'].'</td>
                                			</tr>
                                			<tr>
                                				<td style="color: #333333; font-family: Trebuchet MS; font-size: 14px; padding: 10px 0 0 40px; text-align: left;"><b style="width:100px;display: inline-block;">Alan Adı</b> : '.$res[0]['subdomainurl'].'</td>
                                			</tr>
                                            <tr>
                                				<td style="color: #333333; font-family: Trebuchet MS; font-size: 14px; padding: 10px 0 0 40px; text-align: left;"><b style="width:100px;display: inline-block;">İşlem No</b> : '.$txn_id.'</td>
                                			</tr>
                                            <tr>
                                				<td style="color: #333333; font-family: Trebuchet MS; font-size: 14px; padding: 10px 0 0 40px; text-align: left;"><b style="width:100px;display: inline-block;">Sona Erme Tarihi</b> : '.date('m/d/Y',($upvalidtodate)).'</td>
                                			</tr>
                                            
                                			<tr>
                                				<td style="color: #333333; font-family: Trebuchet MS; font-size: 14px; font-weight: bold; padding: 30px 0 0 10px;" align="left" valign="top" colspan="3">Saygılarımızla,</td>
                                			</tr>
                                			<tr>
                                				<td style="color: #393860; font-family: Trebuchet MS; font-size: 14px; font-weight: bold; padding: 10px 0px 20px 10px;" align="left" valign="top" colspan="3">Enkolayweb.com Ekibi</td>
                                			</tr>
                                		</table>
                                	</body>
                                </html> ';  
    		    ...
    		    $mailsubject  = $_lang['updateSubscriptionDate_sub'];                          	
    ...
    function getBannerSliderImage($domain_id,$status)
        {
            ...
            if($domain_id != '')
                {
                    $sqlsel = "SELECT banner_slider_id,image_name FROM ".$CFG['table']['temp_banner_slider_images']. " WHERE  domain_id ='".$this->filterInput($domain_id)."'  AND status='".$this->filterInput($status)."' ORDER BY banner_slider_id DESC";
        			$sqlres =  $this->ExecuteQuery($sqlsel,'select');
    ...
    function deletePintDomain()
        {
            ...
            if($sqlres)
                $_SESSION['home_suc_msg'] = "Seçilen Alan Adı Eşleştirmesi Silindi";
			echo "success";exit;
        }    
    ...
    function deleteNewDomain()
        {
			...
			$sqlres =  $this->ExecuteQuery($del_qury,'delete');
            if($sqlres)
                $_SESSION['home_suc_msg'] = "Seçilen Yeni Alan Adı Silindi";
			echo "success";exit;
        }   
        ...
        foreach($temp_page_val as $temp=>$value)
            {
                if( !in_array($temp_page_val[$temp]['page_id'],$cur_main_page_id) )
                    ...
                    {
                        $updqury  = "UPDATE ".$CFG['table']['pages']." SET page_parent_id='".$this->filterInput($temp_page_val[$temp]['page_parent_id'])."',team_with_text ='".$this->filterInput($temp_page_val[$temp]['team_with_text'])."',pagename='".$this->filterInput($temp_page_val[$temp]['pagename'])."',seo_title='".$this->filterInput($temp_page_val[$temp]['seo_title'])."',hide_navigation='".$this->filterInput($temp_page_val[$temp]['hide_navigation'])."',page_layout='".$this->filterInput($temp_page_val[$temp]['page_layout'])."',page_desc='".$this->filterInput($temp_page_val[$temp]['page_desc'])."',meta_key='".$this->filterInput($temp_page_val[$temp]['meta_key'])."',footer_code='".$this->filterInput($temp_page_val[$temp]['footer_code'])."',header_code='".$this->filterInput($temp_page_val[$temp]['header_code'])."',social_plugins='".$this->filterInput($temp_page_val[$temp]['social_plugins'])."',title_select='".$this->filterInput($temp_page_val[$temp]['title_select'])."',text_title='".$this->filterInput($temp_page_val[$temp]['text_title'])."',text_description='".addslashes($temp_page_val[$temp]['text_description'])."',description_select='".$this->filterInput($temp_page_val[$temp]['description_select'])."',contact_form='".$this->filterInput($temp_page_val[$temp]['contact_form'])."',divider='".$this->filterInput($temp_page_val[$temp]['divider'])."',image_select='".$this->filterInput($temp_page_val[$temp]['image_select'])."',image_multiple_select='".$this->filterInput($temp_page_val[$temp]['image_multiple_select'])."',image_text='".$this->filterInput($temp_page_val[$temp]['image_text'])."',blog_comments='".$this->filterInput($temp_page_val[$temp]['blog_comments'])."',store_comment='".$this->filterInput($temp_page_val[$temp]['store_comment'])."',buildtitle_bold='".$this->filterInput($temp_page_val[$temp]['buildtitle_bold'])."',buildtitle_italic='".$this->filterInput($temp_page_val[$temp]['buildtitle_italic'])."',buildtitle_underline='".$this->filterInput($temp_page_val[$temp]['buildtitle_underline'])."',buildtitle_strikethrough='".$this->filterInput($temp_page_val[$temp]['buildtitle_strikethrough'])."',buildTitle_textalign='".$this->filterInput($temp_page_val[$temp]['buildTitle_textalign'])."',buildTitle_fontsize='".$this->filterInput($temp_page_val[$temp]['buildTitle_fontsize'])."',buildTitle_fontcolor='".$this->filterInput($temp_page_val[$temp]['buildTitle_fontcolor'])."',buildPara_bold='".$this->filterInput($temp_page_val[$temp]['buildPara_bold'])."',buildPara_italic='".$this->filterInput($temp_page_val[$temp]['buildPara_italic'])."',buildPara_underline='".$this->filterInput($temp_page_val[$temp]['buildPara_underline'])."',buildPara_strikethrough='".$this->filterInput($temp_page_val[$temp]['buildPara_strikethrough'])."',buildPara_textalign='".$this->filterInput($temp_page_val[$temp]['buildPara_textalign'])."',buildPara_fontsize='".$this->filterInput($temp_page_val[$temp]['buildPara_fontsize'])."',buildPara_fontcolor='".$this->filterInput($temp_page_val[$temp]['buildPara_fontcolor'])."',sort_order='".$this->filterInput($temp_page_val[$temp]['sort_order'])."',link_site='".$this->filterInput($temp_page_val[$temp]['link_site'])."',link_status='".$this->filterInput($temp_page_val[$temp]['link_status'])."',banner_status='".$this->filterInput($temp_page_val[$temp]['banner_status'])."' WHERE page_id= '".$this->filterInput($temp_page_val[$temp]['page_id'])."' AND domain_id ='".$this->filterInput($temp_page_val[$temp]['domain_id'])."' ";
        ...
        if($mdStatus =="1" || $mdStatus == "2" || $mdStatus == "3" || $mdStatus == "4")
        	{ 	      
        		...
        		{	
					...
					$user_id                         = $_SESSION['user_id'];            			
					$this->logTransactions($user_id,$_SESSION['domain_id'],$postarrvalues,'Abonelik Ödeme Bekliyor');
					$seourl="/subscription/".$_GET['domain_id']."/failure";
            		$filename="/subscription.php?domain_id=".$_GET['domain_id']."&msg=failure";
            		$redirect_url = ($CFG['site']['userfriendly']=='Y') ? $seourl : $filename;
                	$url= $CFG['site']['base_url'].$redirect_url;
               ...
               {                                   
                   ...
                   echo "<style>
        			   ...
        			   <div id='paymentLoadWindow' class='paymentLoad'  style='display: block;'>
        				<div class='contain'>
        					<div class='payLoader'>
        						Lütfen tarayıcı pencerenizi tazelemeyin veya Geri düğmesine basmayın!						
        						<div class='loadingtxt'>Yükleniyor...</div>
        					</div>
...
function ImageLibrary()
{
      ...
      $sqlsel   = "SELECT `id`, `user_id`, `image_name`, `image_type`, `addeddate` FROM ".$CFG['table']['imagelibrary']." WHERE user_id='".$user_id."'";
      $res  = $this->ExecuteQuery($sqlsel,'select');
      $objSmarty->assign('imageslist',$res);  
...
{
    global $CFG,$objSmarty;
    $domain_list    = $this->getMultiValue("domain_id",$CFG['table']['temp_domaindetails'],"user_id='".$_SESSION['user_id']."'");   
       
    
    $res    = $this->getMultiValue("id,image_name,image_type",$CFG['table']['imagelibrary'],"id='".$_POST['imgid']."'");
    $imgname    = $res[0]['image_name'];
    
    @unlink($CFG['site']['photo_domain_image_path']."/".$imgname);      
    if($res[0]['image_type']=="gallery")    
    {
        $thumbs     = explode(".",$imgname);
        $thumbimg   = $thumbs[0]."T".".".$thumbs[1];
        @unlink($CFG['site']['photo_domain_image_path']."/".$thumbimg);  
    }
    $table1=$CFG['table']['temp_single_images'];
    $tablesub1=$CFG['table']['single_images'];
    

    $table2=$CFG['table']['temp_imagetxt_images'];
    $tablesub2=$CFG['table']['imagetxt_images'];
    

    $table3=$CFG['table']['temp_slider_images'];
    $tablesub3=$CFG['table']['slider_images'];
    
   
    $table4=$CFG['table']['temp_gallery_images'];
    $tablesub4=$CFG['table']['gallery_images'];
            

    $table5=$CFG['table']['temp_banner_slider_images'];
    $tablesub5=$CFG['table']['banner_slider_images'];    

    $table6=$CFG['table']['domain_images'];    
    
    $sqldel1    = "DELETE FROM ".$table1." WHERE imagelib_id='".$_POST['imgid']."'";
    $sqldel2    = "DELETE FROM ".$table2." WHERE imagelib_id='".$_POST['imgid']."'";
    $sqldel3    = "DELETE FROM ".$table3." WHERE imagelib_id='".$_POST['imgid']."'";
    $sqldel4    = "DELETE FROM ".$table4." WHERE imagelib_id='".$_POST['imgid']."'";
    $sqldel5    = "DELETE FROM ".$table5." WHERE imagelib_id='".$_POST['imgid']."'";
    $sqldel6    = "DELETE FROM ".$table6." WHERE imagelib_id='".$_POST['imgid']."'";
    $sqldel7    = "DELETE FROM ".$tablesub1." WHERE imagelib_id='".$_POST['imgid']."'";
    $sqldel8    = "DELETE FROM ".$tablesub2." WHERE imagelib_id='".$_POST['imgid']."'";
    $sqldel9    = "DELETE FROM ".$tablesub3." WHERE imagelib_id='".$_POST['imgid']."'";
    $sqldel10   = "DELETE FROM ".$tablesub4." WHERE imagelib_id='".$_POST['imgid']."'";
    $sqldel11   = "DELETE FROM ".$tablesub5." WHERE imagelib_id='".$_POST['imgid']."'";
    
    $this->ExecuteQuery($sqldel1,"delete");
    $this->ExecuteQuery($sqldel2,"delete");
    $this->ExecuteQuery($sqldel3,"delete");
    $this->ExecuteQuery($sqldel4,"delete");
    $this->ExecuteQuery($sqldel5,"delete");
    $this->ExecuteQuery($sqldel6,"delete");
    $this->ExecuteQuery($sqldel7,"delete");
    $this->ExecuteQuery($sqldel8,"delete");
    $this->ExecuteQuery($sqldel9,"delete");
    $this->ExecuteQuery($sqldel10,"delete");
    $this->ExecuteQuery($sqldel11,"delete");
    
    $sqldel0    = "DELETE FROM ".$CFG['table']['imagelibrary']." WHERE id='".$_POST['imgid']."'";
    $this->ExecuteQuery($sqldel0,"delete");
    $domain_list    = $this->getMultiValue("domain_id",$CFG['table']['temp_domaindetails'],"user_id='".$_SESSION['user_id']."'");     
    //$this->getNumValues($table5,"");
    //print_r($domain_list);exit;
    for($i=0;$i<count($domain_list);$i++)
    {
        $numvalue  = $this->getNumValues($table5,"domain_id='".$domain_list[$i]['domain_id']."'");
        #Checking slider and banner image setting
        if($numvalue==0)
        {
            $sqlupt1 = "UPDATE ".$CFG['table']['temp_domaindetails']." SET header_banner='1',header_slider='0' WHERE domain_id='".$domain_list[$i]['domain_id']."'";
            $sqlupt2 = "UPDATE ".$CFG['table']['domaindetails']." SET header_banner='1',header_slider='0' WHERE domain_id='".$domain_list[$i]['domain_id']."'";
            $this->ExecuteQuery($sqlupt1,"select");
            $this->ExecuteQuery($sqlupt2,"select");
        }
        
        $numvalue  = $this->getNumValues($table6,"domain_id='".$domain_list[$i]['domain_id']."' AND status='domainlogo'");
        if($numvalue==0)
        {
            $sqlupt = "UPDATE ".$CFG['table']['domaindetails']." SET header_logo_config=IF(header_logo_config=2,'1','1') WHERE domain_id='".$domain_list[$i]['domain_id']."'";
            $this->ExecuteQuery($sqlupt,"update");
            $sqlupt1 = "UPDATE ".$CFG['table']['temp_domaindetails']." SET header_logo_config=IF(header_logo_config='2','1','1') WHERE domain_id='".$domain_list[$i]['domain_id']."'";
            $this->ExecuteQuery($sqlupt1,"update");
        }
    }
...
function uploadImage_Library()
{
    ...
    $imgids="";
    foreach($_POST['imgids'] as $key=>$value)
    {
        $imgids.=",".$value;
    ...
    if($imgids!="")
    {
        $libraryimg     = $this->getMultiValue('id,image_name',$CFG['table']['imagelibrary'],"id IN($imgids) AND user_id='".$_SESSION['user_id']."'");             
               
        if($_POST['type']=='single')
        {
            $table=$CFG['table']['temp_single_images'];             
            $numval = $this->getNumValues($table,"page_list_id='".$this->filterInput($page_list_id)."' AND domain_id='".$_SESSION['domain_id']."'");
            if($numval>0)
            {
                $sqlqry = "DELETE FROM ".$table." WHERE page_list_id='".$this->filterInput($page_list_id)."'";
                $this->ExecuteQuery($sqlqry,"delete");
            }                        
            
            $sqlqry = "INSERT INTO ".$table." SET page_id='".$page_id."',domain_id='".$_SESSION['domain_id']."',page_list_id='".$page_list_id."',imagelib_id='".$libraryimg[0]['id']."',image_name='".$libraryimg[0]['image_name']."',publish_status='U',status='single'"; 
            $this->ExecuteQuery($sqlqry,"insert");
        ...
        if($_POST['type']=='imagetext')
        {
            ...
            $sqlqry = "INSERT INTO ".$table." SET page_id='".$page_id."',domain_id='".$_SESSION['domain_id']."',page_list_id='".$page_list_id."',imagelib_id='".$libraryimg[0]['id']."',image_name='".$libraryimg[0]['image_name']."',publish_status='U',status='singletext'"; 
            $this->ExecuteQuery($sqlqry,"insert");
        ...
        if($_POST['type']=='slider')
        {
            ...
            for($i=0;$i<count($libraryimg);$i++)
            {
                $sqlqry = "INSERT INTO ".$table." SET page_id='".$page_id."',domain_id='".$_SESSION['domain_id']."',page_list_id='".$page_list_id."',imagelib_id='".$libraryimg[$i]['id']."',slider_images='".$libraryimg[$i]['image_name']."',publish_status='U'"; 
                $this->ExecuteQuery($sqlqry,"insert");
            }                         
        ...
        if($_POST['type']=='gallery')
        {
            ...
            for($i=0;$i<count($libraryimg);$i++)
            {
                $sqlqry = "INSERT INTO ".$table." SET page_id='".$page_id."',domain_id='".$_SESSION['domain_id']."',page_list_id='".$page_list_id."',imagelib_id='".$libraryimg[$i]['id']."',gallery_name='".$libraryimg[$i]['image_name']."',gallery_name_thumb='".$libraryimg[$i]['image_name']."',publish_status='U'"; 
                $this->ExecuteQuery($sqlqry,"insert");
            }
        ...
        if($_POST['type']=='bannerslider')
        {
            ...
            for($i=0;$i<count($libraryimg);$i++)
            {
                $sqlqry = "INSERT INTO ".$table." SET page_id='".$page_id."',domain_id='".$_SESSION['domain_id']."',imagelib_id='".$libraryimg[$i]['id']."',image_name='".$libraryimg[$i]['image_name']."',status='sliderimage',publish_status='U'"; 
                $this->ExecuteQuery($sqlqry,"insert");
            }
            $sqlupt = "UPDATE ".$CFG['table']['temp_domaindetails']." SET header_banner='0',header_slider='1',publish_status='U' WHERE domain_id='".$_SESSION['domain_id']."'";
            $this->ExecuteQuery($sqlupt,'update');
            
            $delqry = "DELETE FROM ".$CFG['table']['domain_images']." WHERE domain_id='".$_SESSION['domain_id']."' AND status='bannerimage'";
            $this->ExecuteQuery($delqry,'delete');
        ...
        if($_POST['type']=='bannerimage')
        {
            ...
            $numval = $this->getNumValues($table," status='bannerimage' AND domain_id='".$_SESSION['domain_id']."'");         
            if($numval>0)
            {
                $sqlqry = "DELETE FROM ".$table."  WHERE status='bannerimage' AND domain_id='".$_SESSION['domain_id']."'";                
                $this->ExecuteQuery($sqlqry,"delete");         
            }            
            $sqlqry = "INSERT INTO ".$table." SET page_id='".$page_id."',domain_id='".$_SESSION['domain_id']."',imagelib_id='".$libraryimg[0]['id']."',temp_image_name='".$libraryimg[0]['image_name']."',status='bannerimage'"; 
            $this->ExecuteQuery($sqlqry,"insert");         
                       
                     
            $sqlupt = "UPDATE ".$CFG['table']['temp_domaindetails']." SET header_banner='1',header_slider='0',publish_status='U' WHERE domain_id='".$_SESSION['domain_id']."'";
            $this->ExecuteQuery($sqlupt,'update');         
            $delqry = "DELETE FROM ".$CFG['table']['temp_banner_slider_images']." WHERE domain_id='".$_SESSION['domain_id']."'";
            $this->ExecuteQuery($delqry,'delete');
        }
        if($_POST['type']=='background')
        {
            $table=$CFG['table']['domain_images'];
            $numval = $this->getNumValues($table,"status='backgroundimage' AND domain_id='".$_SESSION['domain_id']."'");
            if($numval>0)
            {
                $sqlqry = "DELETE FROM ".$table."  WHERE page_list_id='".$this->filterInput($page_list_id)."'";
                $this->ExecuteQuery($sqlqry,"update");
            }                        
            else
            {
                $sqlqry = "INSERT INTO ".$table." SET page_id='".$page_id."',domain_id='".$_SESSION['domain_id']."',imagelib_id='".$libraryimg[0]['id']."',temp_image_name='".$libraryimg[0]['image_name']."',status='backgroundimage'"; 
                $this->ExecuteQuery($sqlqry,"insert");
            }         
        }
        if($_POST['type']=='background')
        ...
        if($_POST['type']=='columnImageText')
        {
            $table=$CFG['table']['temp_column_text_images'];             
            $numval = $this->getNumValues($table,"page_list_id='".$this->filterInput($page_list_id)."' AND column_id = '".$this->filterInput($_POST['column_id'])."' AND domain_id='".$_SESSION['domain_id']."'");

            if($numval>0)
            {
                $sqlqry = "DELETE FROM ".$table." WHERE page_list_id='".$this->filterInput($page_list_id)."' AND column_id = '".$this->filterInput($_POST['column_id'])."' AND domain_id='".$_SESSION['domain_id']."' AND status='columnImageText' ";
                $this->ExecuteQuery($sqlqry,"delete");
            }                        
            
             $sqlqry = "INSERT INTO ".$table." SET 
                                   page_id         = '".$page_id."',
                                   domain_id       = '".$_SESSION['domain_id']."',
                                   page_list_id    = '".$page_list_id."',
                                   imagelib_id     = '".$libraryimg[0]['id']."',
                                   image_name      = '".$libraryimg[0]['image_name']."',
                                   column_id       = '".$this->filterInput($_POST['column_id'])."',
                                   publish_status  = 'U',
                                   status          = 'columnImageText'"; 
            $this->ExecuteQuery($sqlqry,"insert");
...
public function getGalleryImageNew($page_list_id)
	{
		global $CFG,$objSmarty;
	    $sqlsel = "SELECT gallery_name,gallery_id,page_list_id,page_id FROM ".$CFG['table']['temp_gallery_images']. " WHERE page_list_id = '".$this->filterInput($page_list_id)."' AND domain_id ='".$this->filterInput($_SESSION['domain_id'])."' AND page_id ='".$this->filterInput($_SESSION['page_id'])."'";
		$sqlres =  $this->ExecuteQuery($sqlsel,'select');
...
function buydomainInvoiceStoreDetails()
{
	...
	if($inv_fname != '' && $inv_surname != '' && $domain_id != '' && $inv_for != '' && $inv_district != '')
	{
		$invoice = $this->getValue("id",$CFG['table']['buy_invoice']," user_id = '".$_SESSION['user_id']."' ");
...
function getInvoicebuyDetailsValues($domain_id)
{
	global $CFG,$objSmarty;

	if(isset($domain_id) && !empty($domain_id))
	{
		$invoiceDet = $this->getMultiValue("user_id,fname ,surname ,invoice_for ,company_name ,tax_office ,tax_number ,id_number ,address ,	district ,city ,country",$CFG['table']['buy_invoice']," user_id = '".$_SESSION['user_id']."'");
		if (!isset($invoiceDet) && count($invoiceDet) <= 0)
		{
			 $invoiceUsername = $this->getValue("username",$CFG['table']['register']," user_id = '".$_SESSION['user_id']."'");
		}
	}
	 $objSmarty->assign('invoiceDet',$invoiceDet);
	 $objSmarty->assign('invoiceUsername',$invoiceUsername);
...
#....................................................................................................  
 function changeCaptchaCode(){
        ...
        $objSmarty->assign("pageid", $pageid);
    }   
    
 #....................................................................................................   

 function getColumnTextImages($pageid)    
 {
 	global $CFG,$objSmarty;
   $columnimages = array();
 ...
 #Update Title Details in column text with image
function updateTilte_columnImage()
{
	global $CFG,$objSmarty;
	...
	$textVal    = $this->getValue("img_id",$CFG['table']['temp_column_text_images']," page_id='".$pageid."' AND domain_id='".$domain_id."' AND page_list_id ='".$page_list_id."' AND column_id = '".$column_id."' AND status  = 'columnImageText_title'");

    if(isset($textVal) && !empty($textVal)) 
        {
            $UpQuery  = " UPDATE ".$CFG['table']['temp_column_text_images']." SET column_text_title = '".$title."'
                              WHERE page_list_id ='".$page_list_id."' AND page_id='".$pageid."' AND domain_id='".$domain_id."' AND column_id = '".$column_id."' AND status  = 'columnImageText_title'";
			$UpResult = mysql_query($UpQuery) or die($this->mysql_error($UpQuery));
        } 
        else
        {
...
#Update Desc Details in column text with image
function updateDesc_columnImage()
{
	global $CFG,$objSmarty;
	$pageid       = $this->filterInput($_POST['page_id']);
	$column_id    = $this->filterInput($_POST['column_id']);
	...
	$textVal    = $this->getValue("img_id",$CFG['table']['temp_column_text_images']," page_id='".$pageid."' AND domain_id='".$domain_id."' AND page_list_id ='".$page_list_id."' AND column_id = '".$column_id."' AND status  = 'columnImageText_desc'");

    if(isset($textVal) && !empty($textVal)) 
        {
            $UpQuery  = " UPDATE ".$CFG['table']['temp_column_text_images']." SET column_text_desc = '".$description."'
                              WHERE page_list_id ='".$page_list_id."' AND page_id='".$pageid."' AND domain_id='".$domain_id."' AND column_id = '".$column_id."' AND status  = 'columnImageText_desc'";
			$UpResult = mysql_query($UpQuery) or die($this->mysql_error($UpQuery));
        } 
        else
        {
        	$UpQuery  = " INSERT INTO ".$CFG['table']['temp_column_text_images']." SET 
        	                          ...
        	                          page_list_id      = '".$page_list_id."',
        	                          column_id         = '".$column_id."',
        	                          column_text_desc = '".$description."',
        	                          publish_status    = 'U',
        	                          status            = 'columnImageText_desc' ";
			$InsResult = mysql_query($UpQuery) or die($this->mysql_error($UpQuery));

        }           
}

function deleteColumnTextImageDetails($status,$pagelist_id)
{
	global $CFG,$objSmarty;
    ...
    if($domain_id != '' && $page_list_id != '')
        {
            $UpQuery  = "DELETE FROM ".$CFG['table']['temp_column_text_images']." WHERE page_list_id ='".$page_list_id."' AND page_id='".$pageid."' AND domain_id='".$domain_id."'";
            $UpResult = mysql_query($UpQuery) or die($this->mysql_error($UpQuery));
                               
         }   
}
function getColumnImageTextValue($domain_id,$table_name)
{
    global $CFG;
    $youtupe  = " SELECT img_id,	page_id ,domain_id ,page_list_id ,imagelib_id ,image_name ,column_text_title , column_text_position, column_text_desc ,column_id , 	alignment ,	status ,publish_status ,delete_status FROM ".$table_name." WHERE domain_id='".$this->filterInput($domain_id)."' ";
    $sqlres =  $this->ExecuteQuery($youtupe,'select');
    return $sqlres;         
} 

function columnImageTextProcess($domain_id)
{
    global $CFG;
    $temp_columnImageText_val    = $this->getColumnImageTextValue($domain_id,$CFG['table']['temp_column_text_images']);
    $main_columnImageText_val    = $this->getColumnImageTextValue($domain_id,$CFG['table']['column_text_images']);
    $temp_columnImageText_id     = array();

    foreach($temp_columnImageText_val as $temp=>$value)
        {
            $temp_columnImageText_id[] = $temp_columnImageText_val[$temp]['img_id'];
        }  
    $cur_main_columnImageText_id = array();    
    foreach($main_columnImageText_val as $main=>$value)
        {
            if( !in_array($main_columnImageText_val[$main]['img_id'],$temp_columnImageText_id) )
                {
                    $delet_id.= $main_columnImageText_val[$main]['img_id'].","; 
                }
            else if(in_array($main_columnImageText_val[$main]['img_id'],$temp_columnImageText_id)) 
                {
                    $cur_main_columnImageText_id[] = $main_columnImageText_val[$main]['img_id'];
                }          
        }    
    $delet_id = substr($delet_id,0,-1); 
    if($delet_id != '')
        {                
            $sql_delete="DELETE FROM ".$CFG['table']['column_text_images']." WHERE domain_id = '".$this->filterInput($domain_id)."' AND img_id IN (".$delet_id.") ";
            $UpResult = mysql_query($sql_delete) or die($this->mysql_error($sql_delete));
        }     
    $temp_upd_id = '';
     foreach($temp_columnImageText_val as $sub=>$value)
        {
            if($temp_columnImageText_val[$sub]['publish_status'] == 'U')
                {
                  if( !in_array($temp_columnImageText_val[$sub]['img_id'],$cur_main_columnImageText_id) )
                    {
                       $ins_qry =  " INSERT INTO ".$CFG['table']['column_text_images']." SET ".
                                    " img_id              = '".$temp_columnImageText_val[$sub]['img_id']."',  ".
                                    " page_id             = '".$temp_columnImageText_val[$sub]['page_id']."', ".
                                    " page_list_id        = '".$temp_columnImageText_val[$sub]['page_list_id']."', ".
                                    " domain_id           = '".$temp_columnImageText_val[$sub]['domain_id']."',".
                                    " image_name          = '".$temp_columnImageText_val[$sub]['image_name']."',".
                                    " imagelib_id         = '".$temp_columnImageText_val[$sub]['imagelib_id']."',".
                                    " column_text_title   = '".$temp_columnImageText_val[$sub]['column_text_title']."',".
                                    " column_text_desc    = '".$temp_columnImageText_val[$sub]['column_text_desc']."',".
                                    " column_text_position    = '".$temp_columnImageText_val[$sub]['column_text_position']."',".
                                    " column_id           = '".$temp_columnImageText_val[$sub]['column_id']."',".                    
                                    " status              = '".$temp_columnImageText_val[$sub]['status']."',".                
                                    " publish_status      = '".$temp_columnImageText_val[$sub]['publish_status']."',".           
                                    " delete_status       = '".$temp_columnImageText_val[$sub]['delete_status']."' ";                      
                        $res = $this->ExecuteQuery($ins_qry,'insert'); 
                        
                        $temp_upd_id = $temp_columnImageText_val[$sub]['img_id'].",";   
                    }    
                   else if(in_array($temp_columnImageText_val[$sub]['img_id'],$cur_main_columnImageText_id) && $temp_columnImageText_val[$sub]['publish_status'] == 'U')   
                    {
                        $upd_qry =  " UPDATE ".$CFG['table']['column_text_images']." SET ".                                        
                                    " image_name          = '".$temp_columnImageText_val[$sub]['image_name']."',".
                                    " imagelib_id         = '".$temp_columnImageText_val[$sub]['imagelib_id']."',".
                                    " column_text_title   = '".$temp_columnImageText_val[$sub]['column_text_title']."',".
                                    " column_text_desc    = '".$temp_columnImageText_val[$sub]['column_text_desc']."',".
                                    ...
                                    " publish_status      = '".$temp_columnImageText_val[$sub]['publish_status']."',".           
                                    " delete_status       = '".$temp_columnImageText_val[$sub]['delete_status']."' ".
                           " WHERE  img_id                = '".$temp_columnImageText_val[$sub]['img_id']."' AND  ".
                                    " page_id             = '".$temp_columnImageText_val[$sub]['page_id']."' AND ".
                                    " page_list_id        = '".$temp_columnImageText_val[$sub]['page_list_id']."' AND  ".
                                    " domain_id           = '".$temp_columnImageText_val[$sub]['domain_id']."' ";  
                        $res = $this->ExecuteQuery($upd_qry,'update'); 
        ...
        if($temp_upd_id != '')
            {
                $this->getUpdate($CFG['table']['temp_column_text_images'],"publish_status = 'N'","img_id IN ('".$this->filterInput($temp_upd_id)."')");
            }
} 
