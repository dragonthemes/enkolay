						...
						if($res_ins)
							{
								...
								$mailsubject  = $CFG['site']['sitename']." Kayıt Detayları ";
  	...
  	function forgetPass(){
		...
		if(count($forgetpass[0]['password']) > 0 && count($forgetpass[0]['password']) != 0){
			if($forgetpass[0]['log_status'] == '2'){
				...
				$mailsubject  		= 	$CFG['site']['sitename'].": Şifre ve Hesap Bilgilerini Unuttum ";
			...
			if($_SESSION['theme'] && isset($_SESSION['user_id']) && !empty($_SESSION['user_id']))
				{
					...
					$page_id   = $this->InsertPageDetails($pagename='Ana Sayfa',$seo_title='ana-sayfa',$page_layout='short',$page_desc='home page',$meta_key='Ana Sayfa',$blog_comment='0',$store_comment='0',$setmain='Y');
					//$this->defaultpageList($UpResult,$page_id,$_POST['theme_id']);
                    $this->InsertPageDetails($pagename='Hakkımızda',$seo_title='hakkimizda',$page_layout='tall',$page_desc='',$meta_key='',$blog_comment='0',$store_comment='0',$setmain='N');
					$this->InsertPageDetails($pagename='Bize Ulaşın',$seo_title='bize-ulasin',$page_layout='tall',$page_desc='',$meta_key='',$blog_comment='0',$store_comment='0',$setmain='N');
	...
	function sendMailForTheme($password)
		{
			...
			$mailsubject='Yeni Siteniz İçin Tebrik Ederiz';
	...
	function sendMailForBlog($password)
		{
			...
			$mailsubject='Yeni Blogunuz İçin Tebrik Ederiz';
			$name=$CFG['site']['sitename'];
			$username=$this->getUserName($this->filterInput($_SESSION['user_id']));
			$to_email= $this->getValue("email",$CFG['table']['register'],"username='".$this->filterInput($username)."'");
			$from_email= $this->getValue("admin_email",$CFG['table']['sitesetting'],"domain_id = '1'");
	...
	function sendMailToCommentedUser()
		{
			...
			$mailsubject  = $CFG['site']['sitename']." Yorum Detayları";
	...
	function sendMailToContactForm($domain_id)
		{
			...
			$mailsubject  = $CFG['site']['sitename']." İletişim Detayları";
		...
		public function updateSubscriptionDate($domain_id,$txn_id)
           {
               ...
               $mail_content = '<html>
    		    ...
    		    $mailsubject  = "Enkolyaweb: Alan Adı Ödemesi Başarılı";                          	
