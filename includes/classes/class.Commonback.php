	...
	*/
	...
	function insertMailDetailsForContact($userid,$pagelistid)
		...
		function getImageTextInSubdomain($page_id,$status)
			{
				global $CFG,$objSmarty;
			 	$image = $this->getValue('image_name',$CFG['table']['imagetxt_images'],'page_list_id = "'.$this->filterInput($page_id).'" AND status="'.$this->filterInput($status).'"');
                //print_r($image);
				$objSmarty->assign('images',$image);
			}
	...
	/*	function getImageForBannerSlider($domain_id,$status='domainlogo')
        ...
        function getImageForBannerSlider($domain_id,$status)
			{
				global $CFG,$objSmarty;
			    $image = $this->getMultiValue('image_name,banner_slider_id',$CFG['table']['banner_slider_images'],'domain_id = "'.$this->filterInput($domain_id).'" and status ="'.$this->filterInput($status).'"');
				$objSmarty->assign('sliderimages',$image);
			}
    ...
    function getBannerSliderImage($domain_id,$status)
        {
            ...
            if($domain_id != '')
                {
                    $sqlsel = "SELECT banner_slider_id,image_name FROM ".$CFG['table']['temp_banner_slider_images']. " WHERE  domain_id ='".$this->filterInput($domain_id)."'  AND status='".$this->filterInput($status)."'";
        			$sqlres =  $this->ExecuteQuery($sqlsel,'select');
...
function nestpayMerchantValues($domain_id)
    {
        ...
        if(USER_FRINDLY == 'Y'){    				
            $nestpay_merchant["okUrl"]   = "https://enkolayweb.com/buycredits/".$domain_id;
            $nestpay_merchant["failUrl"] = "https://enkolayweb.com/buycredits/".$domain_id;
		}else{    				
            $nestpay_merchant["okUrl"]   = "https://enkolayweb.com/buyCredits.php?domain_id=".$domain_id;	
            $nestpay_merchant["failUrl"] = "https://enkolayweb.com/buyCredits.php?domain_id=".$domain_id;		
		}        
