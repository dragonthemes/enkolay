...
***/
jQuery.fn.colourPickerNew = function (conf) {
	// Config for plug
	var config = jQuery.extend({
		ico:	jssitebaseUrl+'/images/jquery.colourPicker.gif',
		id:			'selectbgcolor',	// id of colour-picker container
	...
	var colourPickerNew = jQuery('#' + config.id);

	if (!colourPickerNew.length) {
		colourPickerNew = jQuery('<div id="' + config.id + '"></div>').appendTo(document.body).hide();

		// Remove the colour-picker if you click outside it (on body)
		jQuery(document.body).click(function(event) {
			if (!(jQuery(event.target).is('#' + config.id) || jQuery(event.target).parents('#' + config.id).length)) {
				colourPickerNew.hide(config.speed);
			}
		});
	}
	...
	return this.each(function () {
		...
		var icon	= jQuery('<a class="iconbg" href="#"><img src="' + config.ico + '" alt="' + config.openTxt + '" /></a>').insertBefore('#selectbgcolor');
		...
		jQuery('option', select).each(function () {
			...
			loc += '<li><a onclick="colorchange(this)" href="#" title="' 
					+ title 
					+ '" rel="' 
					+ hex 
					+ '" style="background: #' 
		...
		//var iconhide = '<span class="iconbg"><img src="' + config.ico + '" alt="' + config.openTxt + '" /></span>';
		...
		icon.click(function () {
			...
			colourPickerNew.html('<ul>' + loc + '</ul>').css({
				position: 'absolute', 
				left: iconPos.left + 'px', 
				top: iconPos.top + 'px'
			}).show(config.speed);
			// When you click a colour in the colour-picker
			jQuery('a', colourPickerNew).click(function () {
				...
				// If user wants to, change the input's BG to reflect the newly selected colour
				jQuery('.bgboxcolor').css("background","#"+hex);
				if (config.inputBG) {
