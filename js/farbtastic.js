...
jQuery.fn.colourPicker = function (conf) {
	...
	return this.each(function () {
		...
		//var icon	= jQuery('<a class="coloricon" href="#"></a>').insertAfter(select);
		var icon = jQuery('.toolbar_forecolor');
		//var input	= jQuery('<input type="text" name="' + select.attr('name') + '" value="' + select.val() + '" size="6" />').insertAfter(select);
		var input =	jQuery(".forecolorvalnew");
		...
		icon.click(function () {
			...
			var iconPostop	= icon.offset().top + 32;
