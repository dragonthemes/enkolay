<div class="youtubepopclose" onclick=" $('#imageTxtPop').hide();$('.loadmask').hide();"></div>
<div class="sliderPopupHead">
	RESİM+METİN 
</div>
<div class="row-fluid">    
    <div class="span12 slidePopRhtScroll offset0">
		<div id="image_text_Library" class="clearfix library_contentBanner">        
        <ul>
        ...
        <div id="my_computer">
        	<form id="imageform_{$smarty.request.page_list_id}" class="addeditTop addeditbuttonForm form-horizontal SlideUploadForm" method="post" enctype="multipart/form-data" action='ajaxImageUpload.php' >
        		<label id='imageloadbutton_{$smarty.request.page_list_id}' class="input input-file" for="file">
        			<div class="button">
        				<input type='file' name="photos[]" id="photoimg_{$smarty.request.page_list_id}" class="inputfile" size="1" {if $isBlog eq "Y"}onchange="blogImageUpdatingProcess('{$smarty.request.page_list_id}');"{else}onchange="singleImageUpdatingProcess('{$smarty.request.page_list_id}');"{/if} multiple="" />
        				Yeni Resim Yükle
        			</div>
         ...
         <div id="image_library_content" class="clearfix">
            <input type="button" class="upload_btn" value="Seç ve Ekle" onclick="uploadLibraryImages('{$smarty.session.domain_id}','{$smarty.session.page_id}','{$smarty.request.page_list_id}','imagetext');" />
