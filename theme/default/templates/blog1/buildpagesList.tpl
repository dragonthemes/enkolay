				...
				<div class="blogPageInnerBg blogPageThemeBg">
					...
					<div class="footrAlign">
						...
						{$objCommon->getCurrentDomainDetails()}
							...
							{else}
								<div class="contentedit" contenteditable="true" onblur="return updateFooterContent();" onkeydown="return updateFooterContent();" onkeyup="return updateFooterContent();" id="footer_main">{if $settingpage.0.footer_content}{$settingpage.0.footer_content}{else}{$LANG.create_free_website} <a href="http://{$SITE_MAIN_DOMAIN}">{$SITE_MAIN_DOMAIN}</a>{/if}</div>
							{/if}
						</div>
					</div>
							...
							<div class="span3">
								<div name="popupbrowseslider" id="popupbrowseslider">
									...
									<div class="dc marTop10">
										<a class="saveButtonInput" onclick="imageReloadPagelist();$('.loadmask').hide();">Save changes</a>    
									</div>
								</div>
							</div>
							<div class="span9">
