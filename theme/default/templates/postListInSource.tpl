...
{$objCommon->getImageForShowTop($blog_details.0.domain_id,'backgroundimage')}
	...
	<div class="themewrapper5" {if $banner_config}{if $logoimages}style="background:url({$SITE_DOMAIN_IMAGES_URL}/{$logoimages});"{/if}{/if}>
		<div class="themewrapper4Cont wrapperCont5 clearfix">
			<div class="themeLeftSep mainRightPanelInnerTop">
				{if $themename.0.theme_name|lower eq 'store2'}
					<div class="themewrapperRight">
						<div class="theme4Wrapper">
							<div class="buildNavTabBg marTop23">
								<ul class="buildSection">
									{$objCommon->getPages($domain_details.0.domain_id)}
									{section name="pages" loop="$buildpagelist1"}
										<li class="navTabSub">
											<a class="{if $smarty.session.page_id eq $buildpagelist1[pages].page_id}active{/if}"
								href="{$SITE_SOURCE_BASEURL}/{$buildpagelist1[pages].seo_title}" style="{if $blog_details.0.nav_menu_font_style}font-family:{$domain_details.0.nav_menu_font_style};{/if}{if $domain_details.0.nav_menu_font_size}font-size:{$domain_details.0.nav_menu_font_size}px;{/if}{if $domain_details.0.nav_menu_line_size}line-height:{$domain_details.0.nav_menu_line_size}px;{/if}">{$buildpagelist1[pages].pagename}</a>
											<ul>
				...
				<div class="themewrapper2 themewrapper3 blogwrapper1 blogwrapper2">
					...
					{if $themename.0.theme_name|lower neq 'store2'}
						<div class="themewrapperRight">
							<div class="buildNavTabBg blogthemeNavTab">
								<ul class="buildSection blogthemeNavTab_menu">
									{$objCommon->getPages($blog_details.0.domain_id)}
										{section name="pages" loop="$buildpagelist1"}
										<li>
											<a class="{if $smarty.session.page_id eq $buildpagelist1[pages].page_id}active{/if}"
		...
		<div class="blogwrapper1 blogwrapper2 blog2wrapper">
			...
			<div class="blogPageBgInner blogPageBgInnerMiddle clearfix">
				<div class="blogInner">
					{if !empty($listpost) and $buildpagelist.0.blog_comments eq '1'}
					<div class="blogLeft">
				<ul class="no-mar">
						...
						<!--<li class="blog2compress">
											...
											<div class="blogInnComm">
												...
												<div class="addcomment">				
													...
													{ if $getcomments}
														...
														<a href="{$SITE_SOURCE_BASEURL}/postcomment/{$listpost[tit].post_id}/{$listpost[tit].title}" target="_blank">{$objCommon->getCountComment($listpost[tit].post_id)} Comments</a>
														<!--to show count of comments  ends-->
														
													{else}
														<!--to show count of comments  starts-->
														<a href="{$SITE_SOURCE_BASEURL}/postcomment/{$listpost[tit].post_id}/{$listpost[tit].title}" target="_blank">Add comments</a>
														<!--to show count of comments  ends-->
													{/if}
												</div>
												<div class="fb">
													
													{$objCommon->getSharebutton($listpost[tit].domain_id)}
							...
							{$objCommon->ListDragElementsInPostListPage($smarty.session.page_id,$listpost[tit].post_id)}
							{if $listpost[tit].post_id}
								<div class="blogPageBgInner blogPageBgInnerMiddle clearfix">
				...
				<!---for list drag elements for other than blog page start--->
                	</div>
					<div class="blogRight">
                ...
                <!---for list drag elements for other than blog page end----->
				<!--CONTENT END-->
				<div class="footerBg marTop20">
					<!--FOOTER CODE STARTS-->
					<div class="dc">
						{if $blog_details}
