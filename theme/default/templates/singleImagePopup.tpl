<div class="youtubepopclose" onclick=" $('#singlePoP').hide();$('.loadmask').hide();"></div>
...
<div class="row-fluid">    
    <div class="span12 slidePopRhtScroll offset0">
		...
		{*}<a class="upload_btn_save" href="{if $USERFRIENDLY eq 'Y'}main{else}main.php{/if}">Kaydet</a>	*}
         ...
         <div id="image_library_content" class="clearfix">
            <input type="button" class="upload_btn" value="Seç ve Ekle" onclick="uploadLibraryImages('{$smarty.session.domain_id}','{$smarty.session.page_id}','{$smarty.request.page_list_id}','single');" />
