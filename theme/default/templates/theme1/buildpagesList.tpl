...
<div class="{$smarty.session.themename} navigation clearfix {$smarty.session.themecolorname} {if $smarty.session.themename eq 'theme9'}theme9{/if}">
							...
							<div class="mainRhtBanner clearfix">
								<div class="mainRhtBannerInner">
									...
									<div class="mainRhtBannerInnerBanner" {if $logoimages}style="background-image:url({$SITE_DOMAIN_IMAGES_URL}/{$logoimages});"{/if}>
										<a class="editbannerTxt" onclick="$('#editBannerImg').show();$('.loadmask').show();">{$LANG.edit_image_name} </a>
										<a class="editbannerTxt"  onclick="$('#sliderBannerImg').show();$('.loadmask').show();">{$LANG.slider_name} </a>
