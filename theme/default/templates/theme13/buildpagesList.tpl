...
<div class="clearfix {$smarty.session.themecolorname} {if $smarty.session.themename eq 'theme9'}theme9{/if}">
	...
	<div class="theme2Banner" id="theme_background" style="{if $banner_config}{if $logoimages && $default_switch eq 'N'}background:url({$SITE_DOMAIN_IMAGES_URL}/{$logoimages}); background-size:100% 100%;{/if}{else}{if $background_color.0.background_color neq '' && $background_color.0.background_color_off eq 'N' && $default_switch eq 'N'}background-color:{$background_color.0.background_color}{/if}{/if}">
							...
							<div class="mainRhtBanner clearfix">
								...
								<div class="mainRhtBannerInner">
									...
									<div class="mainRhtBannerInnerBanner" {if $logoimages}style="background-image:url({$SITE_DOMAIN_IMAGES_URL}/{$logoimages});"{/if}>
										<a class="editbannerTxt"   onclick="$('#editBannerImg').show();$('#maska').show();">
										{$LANG.edit_image_name} <i class="fa fa-caret-down"></i>
										</a>
										<a class="editbannerTxt theme4editimg"   onclick="$('#sliderBannerImg').show();$('#maska').show();" >
