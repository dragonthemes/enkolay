...
<div class="clearfix {$smarty.session.themecolorname} {if $smarty.session.themename eq 'theme9'}theme9{/if}" id="theme_background" style="{if $banner_config}{if $logoimages && $default_switch eq 'N'}background:url({$SITE_DOMAIN_IMAGES_URL}/{$logoimages}); background-size:100% 100%;{/if}{else}{if $background_color.0.background_color neq '' && $background_color.0.background_color_off eq 'N' && $default_switch eq 'N'}background-color:{$background_color.0.background_color}{/if}{/if}">
		...
		<div class="theme2Banner">
										...
										<div id="banner">
											<div id="bannerleft">
												<div class="wsite-header" {if $logoimages}style="background-image:url({$SITE_DOMAIN_IMAGES_URL}/{$logoimages}); background-size:100% 100%;"{/if}>
													<a class="editbannerTxt" onclick="$('#editBannerImg').show();$('.loadmask').show();">{$LANG.edit_image_name} <i class="fa fa-caret-down"></i></a>
													<a class="editbannerTxt"  onclick="$('#sliderBannerImg').show();$('.loadmask').show();">{$LANG.slider_name} <i class="fa fa-caret-down"></i></a>
												</div>
											...
											<div id="bannerright">
												<div class="bannerrightCont">
													...
													<div id="headingdescription" contenteditable="true"  data-ph="Açıklamayı Güncelle"  class="contentedit {if $site_title.0.heading_description eq ''}clickheretext contenttext{/if}" onblur="updateDescriptionTitle('{$smarty.session.domain_id}')">{if $site_title.0.heading_description}{$site_title.0.heading_description}{/if}</div>
												</div>
											</div>
										</div>
										{/if}
			...
			<div class="blogPageInner">
				<div class="blogPageInnerBg blogPageThemeBg">
					...
					<div class="themewrapper5 theme2Banner themepadnone">
