...
<div class="{$smarty.session.themename} clearfix {$smarty.session.themecolorname} {if $smarty.session.themename eq 'theme9'}theme9{/if}">
	<div class="clear2teheme"></div>
	<div class="themewrapper5 theme2Banner" id="theme_background" style="{if $banner_config}{if $logoimages && $default_switch eq 'N'}background:url({$SITE_DOMAIN_IMAGES_URL}/{$logoimages}); background-size:100% 100%;{/if}{else}{if $background_color.0.background_color neq '' && $background_color.0.background_color_off eq 'N' && $default_switch eq 'N'}background-color:{$background_color.0.background_color}{/if}{/if}">
		<div class="theme5wrap">
			<div class="themewrapper4Cont wrapperCont5 clearfix" id="titheaddesUpdate">
				<div class="themeLeftSep mainRightPanelInnerTop">
								...
								<ul class="buildSection nav_menu blogthemeNavTab_menu" style="{if $site_title.0.nav_menu_font_style}font-family:{$site_title.0.nav_menu_font_style};{/if}{if $site_title.0.nav_menu_font_size}font-size:{$site_title.0.nav_menu_font_size}px;{/if}{if $site_title.0.nav_menu_line_size}line-height:{$site_title.0.nav_menu_line_size}px;{/if}">
									...
									<li class="navTabSub">
										{if $buildpagelist1[pages].seo_title eq 'link'}
											...
											<span class="externalLinkBuildPop" id="externalLinkBuildPop_{$buildpagelist1[pages].page_id}" style="display:none;"><span class="content"></span></span>
										...
										<ul>
											...
											onclick="showPageListinBuild('{$subbuildpagelist[submenu].page_id}','{$subbuildpagelist[submenu].domain_id}')">{$subbuildpagelist[submenu].pagename}</a></li>
											{/section}
										</ul>
									</li>
									{/section}
					...
					<div class="logoTopHoverOuter">
						<div class="themewrapper1 themewrapper3 blogwrapper1  blogwrapper2">
							<div class="theme4Wrapper">
										...
										<div class="themewrapper3">
											<div class="mainRhtBannerInnerRight">
												<h2 class="mainRhtBannerInnerHead main_headline" style="{if $site_title.0.main_headline_font_style}font-family:{$site_title.0.main_headline_font_style};{/if}{if $site_title.0.main_headline_font_size}font-size:{$site_title.0.main_headline_font_size}px;{/if}{if $site_title.0.main_headline_line_size}line-height:{$site_title.0.main_headline_line_size}px;{/if}">
													<div id="headingContent"  contenteditable="true"  data-ph="Başlığı Güncelle"  class="minhead headlimit contentedit {if $site_title.0.heading_content eq ''}clickheretext contenttext{/if}" onchange="updateHeading('{$smarty.session.domain_id}')"   onblur="updateHeading('{$smarty.session.domain_id}')" onchange="updateHeading('{$smarty.session.domain_id}')"  >{if $site_title.0.heading_content}{$site_title.0.heading_content}{/if}</div>
												</h2>
			...
			<div class="blogPageInner clearfix">
